# Jeffrey Mays
# AEM 517 - Spring 2020
# Project 4: INS/GNSS Error State Extended Kalman Filter
# Due April 27, 2020
...
old_time = 0.0                                  # current time
old_latR = 33.189281 * DTOR                     # lat is first GPS reading
old_longR = -87.565155 * DTOR                   # Long is first GPS reading
old_alt = 67.6                                  # alt is first GPS reading
old_eul_nb = np.zeros((3, 1))                   # Euler angles are [0 0 0]^T
old_C_b_n = trans.Euler_to_CTM(old_eul_nb).T    # old CTM
old_v_eb_n = np.array([0.0, 0.0, 0.0])          # vel ned is first GPS reading
first_omega_ib_b = np.array([0.0, 0.0, 0.0])    # first gyro measurement
first_f_ib_b = np.array([0.0, 0.0, 0.0])        # first accel measurement
...
for epoch in range(1, no_epochs):
    ...
    time = time + dt
    meas_latR = (33.189281 * DTOR) + ((random() - 0.5)*2) / 10000000
    meas_longR = (-87.565155 * DTOR) + ((random() - 0.5)*2) / 10000000
    meas_alt = 67.6 + ((random() - 0.5)/2) * 2.0
    meas_v_eb_n = np.zeros((3))
    meas_v_eb_n[0] = 0.0 + ((random() - 0.5)*2)
    meas_v_eb_n[1] = 0.0 + ((random() - 0.5)*2)
    meas_v_eb_n[2] = 0.0 + ((random() - 0.5)*2)
    meas_omega_ib_b = np.zeros((3))
    meas_omega_ib_b[0] = (((random() - 0.5)*2)) * 3.0 * DTOR
    meas_omega_ib_b[1] = (((random() - 0.5)*2)) * 3.0 * DTOR
    meas_omega_ib_b[2] = (((random() - 0.5)*2)) * 3.0 * DTOR
    meas_f_ib_b = np.zeros((3))
    meas_f_ib_b[0] = (((random() - 0.5)*2)) * 2.0
    meas_f_ib_b[1] = (((random() - 0.5)*2)) * 2.0
    meas_f_ib_b[2] = (((random() - 0.5)*2)) * 2.0 + 9.81
    ...
    new_meas_latR = (33.189281 * DTOR) + ((random() - 0.5)*2) / 10000000
    new_meas_longR = (-87.565155 * DTOR) + ((random() - 0.5)*2) / 10000000
    new_meas_alt = 67.6 + ((random() - 0.5)/2) * 2.0
    new_meas_v_eb_n = (((random() - 0.5)*2)) * 5.0 * DTOR
    ...
    P, FP = EKF.prediction_step(F, P, Q)
    ...
    if (time - time_last_GNSS) >= GNSS_epoch_interval:
        ...
        est_latR, est_longR, est_alt, est_v_eb_n, est_C_b_n, est_IMU_bias = EKF.correct_NED_errors(delta_x, est_C_b_n, est_IMU_bias, old_latR, est_latR, est_longR, est_alt, new_meas_alt, est_v_eb_n, new_meas_v_eb_n)
